export default class UIManager {
  ...
  constructor(scene) {
    ...
    this.damageFlash =
      scene.add.rectangle(scene.scale.width / 2, scene.scale.height / 2, scene.scale.width, scene.scale.height, 0xff0000, 0)
        .setScrollFactor(0)
        .setDepth(25);
  ...
  createGameOverUI() {
    ...
    this.gameOverGroup = this.scene.add.container(this.scene.scale.width / 2, this.scene.scale.height / 2)
      .setScrollFactor(0)
      .setDepth(40)
      .setVisible(false);

    const overlay =
      this.scene.add.rectangle(0, 0, this.scene.scale.width, this.scene.scale.height, 0x000000, 0.65);
  ...
  createStartMenu() {
    this.startMenuGroup = this.scene.add.container(this.scene.scale.width / 2, this.scene.scale.height / 2)
      .setScrollFactor(0)
      .setDepth(100);

    const overlay = this.scene.add.rectangle(0, 0, this.scene.scale.width, this.scene.scale.height, 0x000000, 0.65);
  ...
  createMobileControls(touchState) {
    ...
    // Only show controls on non-desktop devices or if we are in portrait mode
    const isMobile = !this.scene.sys.game.device.os.desktop || isPortrait;
    if (!isMobile) return;

    // Sized up from 42 to 55 for comfortable mobile thumb inputs
    const btnRadius = 55;
    const createButton = (x, y, label, down, up) => {

      const button =
        this.scene.add.circle(x, y, btnRadius, 0x173c50, 0.75)
    ...
    // Calculate Y offset: in portrait, place them slightly higher (height - 200) to ensure visibility and prevent clipping
    const buttonY = isPortrait ? height - 200 : height - 120;

    // Jump button on the left - positioned for ergonomics
    createButton(
      180,
      buttonY,
      "^",
