...
export default class DayNightManager {
  ...
  constructor(scene) {
    ...
    this.nightOverlay =
      scene.add.rectangle(scene.scale.width / 2, scene.scale.height / 2, scene.scale.width, scene.scale.height, 0x0a1929, 0)
        .setScrollFactor(0)
        .setDepth(20);

    // More detailed sun with glow effect
    this.sun =
      scene.add.circle(scene.scale.width - 180, 120, 45, 0xffdd55, 1)
        .setScrollFactor(0)
        .setDepth(5);
    this.sunGlow =
      scene.add.circle(scene.scale.width - 180, 120, 65, 0xffdd55, 0.3)
        .setScrollFactor(0)
        .setDepth(4);

    // More detailed moon with glow effect
    this.moon =
      scene.add.circle(scene.scale.width - 180, 120, 35, 0xe8f1ff, 1)
        .setScrollFactor(0)
        .setDepth(6);
    this.moonGlow =
      scene.add.circle(scene.scale.width - 180, 120, 50, 0xe8f1ff, 0.2)
        .setScrollFactor(0)
        .setDepth(5);
